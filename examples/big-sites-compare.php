...
foreach ($sites as $siteName => $siteUrl) {
    ...
    echo str_pad((string) number_format($sourceLen), 10, ' ', STR_PAD_LEFT);
    echo ' | ';
    $start = microtime(true);
    $compressed = $parser->compress($source);
    $end = microtime(true);
    $compressedLen = strlen($compressed);
    echo str_pad((string) number_format($compressedLen), 10, ' ', STR_PAD_LEFT);
    echo ' | ';
    echo str_pad((string) number_format($sourceLen - $compressedLen), 10, ' ', STR_PAD_LEFT);
    echo ' | ';
    echo str_pad((string) round(($compressedLen / $sourceLen) * 100, 3) . '%', 10, ' ', STR_PAD_LEFT);
    echo ' | ';
    echo str_pad((string) str_pad((string) round($end - $start, 5), 7, ' ') . ' sec', 11, ' ', STR_PAD_LEFT);
    echo ' |';
    echo PHP_EOL;
    echo $line;
    echo PHP_EOL;
